...
class ReadThread(Thread):
    ...
    def __write(self, task):
        ...
        if barcodes not in self.__files:
            ...
            # >> an ugly relealization of unlimited files.
            # >> can be replaced by `import resource`
            filename = self.__files[barcodes].name

# =============================================================================
#             self.__files[barcodes] = open(filename, 'w')
# 
#             SeqIO.write(task[1], self.__files[barcodes], 'fasta') # one tab, by zhou_yiwei
#         
#             self.__files[barcodes].close() # close the file, by zhou_yiwei
# =============================================================================
        # >> an ugly relealization of unlimited files.
        # >> can be replaced by `import resource`
        with open(filename, 'a') as f_h:
          self.__files[barcodes] = f_h
          SeqIO.write(task[1], self.__files[barcodes], 'fasta') # one tab, by zhou_yiwei
...
def demultiplex(barcodes, in_dir, min_length, max_read_files, out_dir,
    ...
    filenames = utils.get_filenames([in_dir], max_read_files)
